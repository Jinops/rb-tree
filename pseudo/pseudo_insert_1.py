insert(tree, key):
  ...
  _insert_case_1(node)로 전체 검증을 시작한다.
...
_insert_case_3(node): 
  ...
  if 삼촌이 red라면:
    ...
    _insert_case_1(조부모) 검증을 시행한다.
  else:
    _insert_case_4(node) 검증을 시행한다.

_insert_case_4(node): 
  # 부모는 red, 삼촌은 black인 경우, rotate를 한다.
  if node가 부모의 왼쪽 자식이고, 부모는 조부모의 오른쪽 지식이면:
    rotate_right(부모)를 시행한다.
    node의 역할을 오른쪽 자식이 대신한다. # node=node.right
  else if node가 부모의 오른쪽 자식이고, 부모는 조부모의 왼쪽 자식이면:
    rotate_left(부모)를 시행한다.
    node의 역할을 왼쪽 자식이 대신한다. # node=node.left
  
  _insert_case_5(node) 검증을 시행한다.
